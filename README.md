...
3. **Position balance** — per-position counts differ by at most 1; out-of-position placements are legal only at a player's secondary position, flagged, and shared between teams. Each team fields a keeper, a defender and a midfielder whenever the squad allows — and "allows" counts deputies, so a night when no specialist keeper turns up still puts someone in each goal rather than leaving one empty.
